...
export default function LocalScreen({ navigation, route }) {
  ...
  const Send_ACK = async (id) => {
    await manager.writeCharacteristicWithResponseForDevice(
      id,
      myServiceUUID,
      device_ch,
      encode(String.fromCharCode(99))
    )
  }
    ...
    try {
      if (connectedDevice.length > 0) {
        ...
        await manager.writeCharacteristicWithResponseForDevice(
          ...
          myServiceUUID,
          device_ch,
          encode(String.fromCharCode(98)+code)
        )
      } else {
    ...
    <View style={{ flex: 1, backgroundColor: 'white' }}>
      ...
      <View style={{ flex: 4, alignItems: 'center', justifyContent: 'center' }}>
        ...
        <View style={{ height: height / 2, width: width / 1.2, borderWidth: 1, borderRadius: 30 }}>
          ...
          <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center' }}>
            <View style={styles.viewText}>
              <TouchableOpacity style={styles.Opacity} onPress={() => requestData(password)}>
