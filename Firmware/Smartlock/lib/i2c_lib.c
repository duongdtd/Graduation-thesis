...
void i2c_init (void)
{
  ...
  // Route I2C pins to GPIO
   ...
   GPIO->I2CROUTE[0].SCLROUTE |= (I2C_SCL_PORT << 0)|(I2C_SCL_PIN << 16);
   GPIO->I2CROUTE[0].SDAROUTE |= (I2C_SDA_PORT << 0)|(I2C_SDA_PIN << 16);
...
void i2c_write_nBytes (uint16_t deviceAddress,uint8_t *txData, uint8_t numBytes)
{
  I2C_TransferSeq_TypeDef i2cTransfer;
  I2C_TransferReturn_TypeDef result;
...
void i2c_writeBit (uint8_t deviceAddress, uint8_t regAddress, uint8_t bit_n,
           uint8_t data)
{
  uint8_t b;
